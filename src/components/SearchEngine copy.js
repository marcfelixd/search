...
import { X, ChevronLeft, ChevronRight, Search, MessageSquare, Send, Settings } from 'lucide-react';
...
const SearchEngine = () => {
  ...
  const performSearch = async (searchQuery = query) => {
    ...
    try {
      ...
      const searchResults = await apiFunction(searchQuery);
    ...
    <div className="min-h-screen flex flex-col p-4 relative" style={{ backgroundColor: colors.background, color: colors.text }}>
      ...
      <motion.div 
        ...
        >
            ...
            <motion.div
            className="flex-shrink-0 mt-4" // Ajoutez la marge supérieure pour créer un espace entre le bot et le logo
            initial={false}
            animate={{
                alignSelf: hasSearched ? 'flex-start' : 'center',
                marginBottom: hasSearched ? 0 : '1rem',
                scale: hasSearched ? 0.75 : 1,
                position: hasSearched ? 'absolute' : 'relative',
                left: hasSearched ? '20px' : 'auto',
                top: hasSearched ? '20px' : 'auto',
            }}
            transition={{ duration: 0.5, ease: "easeInOut" }}
            >
            <Logo />
